...
| SO-100 warm-start ablation | 🚫 Launch blocked | `lewm-train` now consumes `training.warmstart_from`, `jobs/train_so100_warmstart.yaml` exists with a fail-closed source-checkpoint env gate, the safety leash marks it approval-required, and a local one-step writer smoke proves the current 41,856-param bounded source contract; `reports/pusht_warmstart_hub_audit.json` confirms all six public PushT `.mpk` candidates are incompatible by schema, record kind, and/or parameter count, including the 50k `pusht-full-lewm-*` candidate (`minimal-lewm`, 56 params), so no trained compatible bounded source checkpoint is published and paid launch still requires human approval; see `reports/so100_warmstart.md` |
...
| Phase A handoff | ✅ Local gate | `reports/phase_a_handoff.json` records the ordered F1/F3 commands, human-approval gates, source prefixes, template placeholders, F1 GHCR/source-build alternatives, the concrete source-build dry-run report, final `onnx-full/` upload approval via `--allow-hub-upload`, and acceptance evidence; `scripts/check_phase_a_handoff.py` and `scripts/check_f1_source_build_dry_run_report.py` validate it in `make check` |
| Phase A paid approval packet | ✅ Local gate | `reports/phase_a_approval.json` records F1 worst-case `$18.00`, F3 worst-case `$9.00`, the `$27.00` combined spend exceeding the `$20.00` session cap, the F1 source-build fallback with a concrete preflighted revision, and the F3 template replacement rule; `scripts/check_phase_a_approval.py` validates it in `make check` |
