...
| SO-100 warm-start ablation | 🚫 Launch blocked | `lewm-train` now consumes `training.warmstart_from`, `jobs/train_so100_warmstart.yaml` exists with a fail-closed source-checkpoint env gate, the safety leash marks it approval-required, and a local one-step writer smoke proves the current 41,856-param bounded source contract; no trained compatible bounded PushT source checkpoint is published and paid launch still requires human approval; see `reports/so100_warmstart.md` |
