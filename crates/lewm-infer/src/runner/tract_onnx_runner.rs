//! ONNX runner backed by `tract-onnx`.
...
/// Inference runner for `encoder.onnx` and `predictor.onnx`.
#[derive(Debug)]
...
impl TractOnnxRunner {
    /// Load and optimize an ONNX graph pair from a checkpoint directory.
    ///
    /// # Errors
    ///
    ...
    pub fn new(checkpoint_dir: &Path) -> Result<Self, RunnerError> {
        let encoder_path = require_graph(checkpoint_dir.join("encoder.onnx"))?;
        let predictor_path = require_graph(checkpoint_dir.join("predictor.onnx"))?;
        let intra_op_threads = available_intra_op_threads();
        let encoder = load_onnx_graph(&encoder_path, intra_op_threads)?;
        let predictor = load_onnx_graph(&predictor_path, intra_op_threads)?;
        Ok(Self {
            encoder,
            predictor,
            metadata: RunnerMetadata {
...
impl InferenceRunner for TractOnnxRunner {
    fn encode(&mut self, pixels: &[f32; IMAGE_ELEMENT_COUNT]) -> Result<Vec<f32>, RunnerError> {
        let input = Tensor::from_shape(&[1, 3, 224, 224], pixels).map_err(|error| {
            RunnerError::Backend {
                context: "building ONNX encoder input tensor".to_owned(),
    ...
    fn predict(
        ...
        let history = Tensor::from_shape(&[1, h, latent_dim], history).map_err(|error| {
            RunnerError::Backend {
                context: "building ONNX predictor history tensor".to_owned(),
                source: error.to_string(),
            }
        })?;
        let actions =
            Tensor::from_shape(&[1, h, a], actions).map_err(|error| RunnerError::Backend {
                context: "building ONNX predictor action tensor".to_owned(),
                source: error.to_string(),
            })?;
        run_one_output(
            self.metadata.format,
...
fn load_onnx_graph(path: &Path, intra_op_threads: usize) -> Result<Runnable, RunnerError> {
    ...
    let model = tract_onnx::onnx()
        .model_for_path(path)
        .map_err(|error| onnx_backend_error(path, &error))?;
    let model = model
        .into_optimized()
        .map_err(|error| onnx_backend_error(path, &error))?;
    model
...
fn onnx_backend_error(path: &Path, error: &TractError) -> RunnerError {
    RunnerError::Backend {
        context: format!("loading optimized ONNX graph {}", path.display()),
        source: error.to_string(),
    }
}
