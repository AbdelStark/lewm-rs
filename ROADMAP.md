...
| GPU inference (Burn) | Implemented | `lewm_infer::runner::BurnJepaRunner<B>` runs the Burn `Jepa<B>` module directly; feature `burn-cpu` (NdArray, on by default) + `burn-cuda` (NVIDIA GPU, opt-in) compile clean across the workspace; `lewm-infer --backend burn-cpu` / `--backend burn-cuda` selects the backend at runtime; CI verifies `cargo clippy --workspace --all-targets --all-features` is warning-free |
